...
@triton.jit
def _bmm_chunk_fwd_kernel_persistent(
    grid_atomic,
    # Pointers to matrices
    a_ptr, b_ptr, out_ptr, seq_idx_ptr,
    # Matrix dimensions
    seqlen, chunk_size, K, ngroups, batch, nchunks,
    stride_a_batch, stride_a_seqlen, stride_a_head, stride_ak,
    stride_b_batch, stride_b_seqlen, stride_b_head, stride_bk,
    stride_out_batch, stride_out_chunk, stride_out_head, stride_outm, stride_outn,
    stride_seq_idx_batch, stride_seq_idx_seqlen,
    ...
    grid_dim_N = tl.cdiv(chunk_size, BLOCK_SIZE_N)
    target_grid_size = grid_dim_M * grid_dim_N * batch * (nchunks * ngroups)
    ...
    while(my_block_id < target_grid_size):
        pid_h = my_block_id % ngroups
        pid_c = (my_block_id // ngroups) % nchunks
        pid_b = (my_block_id // (ngroups * nchunks)) % batch
        pid_n = (my_block_id // (ngroups * nchunks * batch)) % grid_dim_N
        pid_m = (my_block_id // (ngroups * nchunks * batch * grid_dim_N)) # last one doesn't need mod
        ...
        if IS_CAUSAL:
            if pid_n * BLOCK_SIZE_N >= (pid_m + 1) * BLOCK_SIZE_M:
                my_block_id = tl.atomic_add(grid_atomic, 1).to(tl.int32)
                continue
        a_ptr = a_ptr_base + pid_b * stride_a_batch + pid_c * chunk_size * stride_a_seqlen + pid_h * stride_a_head
        b_ptr = b_ptr_base + pid_b * stride_b_batch + pid_c * chunk_size * stride_b_seqlen + pid_h * stride_b_head
        if HAS_SEQ_IDX:
        ...
        out_ptr = out_ptr_base + pid_b * stride_out_batch + pid_c * stride_out_chunk + pid_h * stride_out_head
        out_ptrs = out_ptr + (stride_outm * offs_m[:, None] + offs_n[None, :] * stride_outn)
        tl.store(out_ptrs, out, mask=(offs_m[:, None] < chunk_size) & (offs_n[None, :] < chunk_size))
        my_block_id = tl.atomic_add(grid_atomic, 1).to(tl.int32)
...
def _bmm_chunk_fwd_persistent(a, b, chunk_size, seq_idx=None, causal=False, output_dtype=None):
    """
    Argument:
        a: (batch, seqlen, k) or (batch, seqlen, ngroups, k)
        b: (batch, seqlen, k) or (batch, seqlen, ngroups, k)
    ...
    with torch.cuda.device(a.device.index):
        _bmm_chunk_fwd_kernel_persistent[actual_grid_size](
            grid_atomic,
            a, b, out, seq_idx,
            seqlen, chunk_size, k, ngroups if has_groups else 1, batch, nchunks,
            a.stride(0), a.stride(1), 0 if not has_groups else a.stride(2), a.stride(-1),
            b.stride(0), b.stride(1), 0 if not has_groups else b.stride(2), b.stride(-1),
            out.stride(0), out.stride(1), 0 if not has_groups else out.stride(2), out.stride(-2), out.stride(-1),
            *((seq_idx.stride(0), seq_idx.stride(1)) if seq_idx is not None else (0, 0)),
