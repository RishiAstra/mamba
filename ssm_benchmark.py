...
def run_original_ssd(x, dt, A, B, C, chunk_size, D, z, dt_bias, initial_states, seq_idx, cu_seqlens, dt_softplus):
    outputs = _mamba_chunk_scan_combined_fwd(x, dt, A, B, C, chunk_size, D=D, z=z, dt_bias=dt_bias, initial_states=initial_states, seq_idx=seq_idx, cu_seqlens=cu_seqlens, dt_softplus=dt_softplus, use_fused5_ssd=False)
...
def run_fused_ssd(x, dt, A, B, C, chunk_size, D, z, dt_bias, initial_states, seq_idx, cu_seqlens, dt_softplus):
    outputs = _mamba_chunk_scan_combined_fwd(x, dt, A, B, C, chunk_size, D=D, z=z, dt_bias=dt_bias, initial_states=initial_states, seq_idx=seq_idx, cu_seqlens=cu_seqlens, dt_softplus=dt_softplus, use_fused5_ssd=True)
...
def get_rand_input(dims_b_seq_nh_hd_ng_ds, is_original=True):
    ...
    if have_z:
        ...
        z = None
    if have_init_states:
        initial_states = torch.randn((batch, nheads, headdim, dstate), dtype=torch.float16 if not states_fp_32 else torch.float32, device=DEVICE) * 0.2
    else:
        initial_states = None
