...
def _fused5_ssd_kernel(
    ...
    * HAS_Z is not tested, NEED_MASK_*=True is not tested, and this kernel was only tested on an A100 and H100.
    ...
    for pid_cs in range(0, cs_num_pid_cs, 1):
        ...
        # TODO: make parameter or something to set if we overwrite x, warn mutated
        out_ptrs = x_ptr + (stride_out_seqlen * offs_cs[:, None] + offs_hd[None, :] * stride_out_hdim)
        tl.store(out_ptrs, acc, mask=(offs_cs[:, None] < chunk_size_limit) & (offs_hd[None, :] < hdim), cache_modifier='.wt')
