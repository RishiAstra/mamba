...
@triton.jit
def _chunk_cumsum_fwd_persistent_kernel(
    grid_atomic,
    # Pointers to matrices
    dt_ptr, A_ptr, dt_bias_ptr, dt_out_ptr, dA_cumsum_ptr,
    # Matrix dimension
    batch, seqlen, nheads, chunk_size, nchunks,
    dt_min, dt_max,
    # Strides
    stride_dt_batch, stride_dt_seqlen, stride_dt_head,
    stride_A_head,
    ...
    grid_dim_heads = tl.cdiv(nheads, BLOCK_SIZE_H)
    target_grid_size = batch * nchunks * grid_dim_heads

    dt_ptr_base = dt_ptr
    dt_out_ptr_base = dt_out_ptr
    dA_cumsum_ptr_base = dA_cumsum_ptr

    my_block_id = tl.atomic_add(grid_atomic, 1).to(tl.int32)

    while(my_block_id < target_grid_size):
        pid_h = my_block_id % grid_dim_heads
        pid_c = (my_block_id // grid_dim_heads) % nchunks
        pid_b = (my_block_id // (grid_dim_heads * nchunks)) # don't need last mod


        dt_ptr          = dt_ptr_base + pid_b * stride_dt_batch + pid_c * chunk_size * stride_dt_seqlen
        dt_out_ptr      = dt_out_ptr_base + pid_b * stride_dt_out_batch + pid_c * stride_dt_out_chunk
        dA_cumsum_ptr   = dA_cumsum_ptr_base + pid_b * stride_dA_cs_batch + pid_c * stride_dA_cs_chunk
...
@triton.jit
def _chunk_state_fwd_kernel_persistent(
    grid_atomic,
    # Pointers to matrices
    x_ptr, b_ptr, states_ptr, dt_ptr, dA_cumsum_ptr, seq_idx_ptr,
    # Matrix dimensions
...
def _chunk_cumsum_fwd_persistent(dt, A, chunk_size, dt_bias=None, dt_softplus=False, dt_limit=(0.0, float("inf"))):
    batch, seqlen, nheads = dt.shape
    assert A.shape == (nheads,)
    if dt_bias is not None:
        assert dt_bias.shape == (nheads,)
    ...
    with torch.cuda.device(dt.device.index):
        _chunk_cumsum_fwd_persistent_kernel[actual_grid_size](
            grid_atomic,
            dt, A, dt_bias, dt_out, dA_cumsum,
            batch, seqlen, nheads, chunk_size, nchunks,
            dt_limit[0], dt_limit[1],
...
def _chunk_state_fwd_persistent(B, x, dt, dA_cumsum, seq_idx=None, states=None, states_in_fp32=True):
    batch, seqlen, nheads, headdim = x.shape
    _, _, nchunks, chunk_size = dt.shape
    _, _, ngroups, dstate = B.shape
    assert nheads % ngroups == 0
    ...
    grid_atomic = torch.zeros((1), device="cuda")
    with torch.cuda.device(x.device.index):
        _chunk_state_fwd_kernel_persistent[actual_grid_size](
            grid_atomic,
            x, B, states, dt, dA_cumsum, seq_idx,
            headdim, dstate, chunk_size,
